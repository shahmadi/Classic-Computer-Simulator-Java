/*
 this class is for designing fully associative and unified cache 
 */
package frontpanel;
...
public class Cache {
	...
	private CacheSlot find(int memoryAddress) {
		for(int i = 0; i < cacheSize; i++){
			if(cacheSlots.get(i) != null && cacheSlots.get(i).getMemoryAddress() == memoryAddress){
				//.out.println("Memory address " + memoryAddress + " found in Cache...");
				return cacheSlots.get(i);
			}
		}
		//.out.println("Memory address " + memoryAddress + " is NOT in Cache...");
		return null;
	}
	...
	private String bringMemoryAddressToCache(int memoryAddress){
		...
		if(cacheSize == MEMORY_SIZE){
			//.out.println("Bringing data from memory position " + memoryAddress + " to cache position " + memoryAddress);
			cacheSlots.set(memoryAddress, slot);
			return data;
		} else {
			cacheSlots.set(addCounter, slot);
			//.out.println("Bringing data from memory address position " + memoryAddress + " to position " + addCounter);
			incrementAddCounter();
			return data;
		}
	}
